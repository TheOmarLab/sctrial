...
# Core methods
CORE_METHODS = [
    "sctrial_fe", "edger_qlf", "limma_voom", "dreamlet", "nebula", "wilcoxon_paired",
]
...
def _run_single_iteration(args: tuple) -> list[dict]:
    ...
    for method in methods:
        ...
        try:
            ...
            logger.warning("Method %s failed on %s iter %d: %s",
                           method, scenario_name, iteration, exc)
            results = {
...
def _dispatch_method(method: str, sim: dict, gene_cols: list[str]) -> dict:
    ...
    if method == "sctrial_fe":
        ...
        return run(pb_counts, gene_cols)
            ...
            if n_jobs == 1:
                ...
                with mp.Pool(n_jobs) as pool:
                    for i, batch in enumerate(
                        pool.imap(_run_single_iteration, task_args)
                    ):
                        all_rows.extend(batch)
                        if (i + 1) % 20 == 0:
                            elapsed = time.time() - t0
                            eta = elapsed / (i + 1) * (n_iterations - i - 1)
                            print(f"    {i+1}/{n_iterations} iterations "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="NatMeth benchmark: simulation grid"
    )
    ...
    parser.add_argument("--designs", nargs="+", default=["two_arm", "single_arm"],
                        help="Design families")
    parser.add_argument("--methods", nargs="+", default=None,
                        help="Methods to run (default: all core)")

...
# ═══════════════════════════════════════════════════════════════════
_EFFICACY_MAP = {"CR": "R", "PR": "R", "PD": "NR", "SD": "NR"}


def _load_clinical(mmc3_path: Path) -> pd.Series:
    """Return patient → 'R'/'NR' Series read from mmc3.xlsx."""
    ...
    return pt_eff.map(_EFFICACY_MAP)
...
def main():
    ...
    # ── 4b. Add clinical response from mmc3.xlsx ─────────────────
    # CR/PR → R, PD/SD → NR. NaN for patients with 'Na' efficacy.
    mmc3_path = DATA_DIR / "mmc3.xlsx"
    tnbc_response = _load_clinical(mmc3_path)

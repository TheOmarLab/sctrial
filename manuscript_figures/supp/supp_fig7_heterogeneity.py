...
def _panel_loo_influence(ax, effects: pd.DataFrame, features: list[str], title: str):
    ...
    n = mat.shape[0]
    if n < 4:
        ax.text(0.5, 0.5, "Too few participants", transform=ax.transAxes, ha="center", va="center")
        ax.axis("off")
        return
...
def _panel_variance_decomposition(
    ax, effects: pd.DataFrame, features: list[str], title: str, arm_treated: str, arm_control: str
):
    ...
    if len(unique_arms) < 2:
        ax.text(
            0.5,
            0.5,
            "Need two arms for decomposition",
            transform=ax.transAxes,
            ha="center",
            va="center",
            fontsize=9,
    ...
    if len(feat_labels) == 0:
        ax.text(0.5, 0.5, "No variance data", transform=ax.transAxes, ha="center", va="center")
        ax.axis("off")
        return
...
def _panel_cross_dataset_sd(ax, results: dict):
    ...
    if len(sd_data) < 2:
        ax.text(0.5, 0.5, "Need >= 2 datasets", transform=ax.transAxes, ha="center", va="center")
        ax.axis("off")
        return
...
def _panel_effect_direction_diversity(ax, effects: pd.DataFrame, features: list[str], title: str):
...
def _generate_old():
    """DEPRECATED: old 10-panel generate. Replaced by generate() below."""
    pass
...
def generate():
    ...
    Reorganised layout (8 panels):
      A  Individual effects strip (SF)   (was SF7-A)
      B  Participant x feature heatmap   (was SF7-B)
      C  Variance decomposition (SF)     (was SF7-F)
      D  Cross-dataset SD scatter        (was SF7-G)
      E  Effect correlation matrix (SF)  (was SF7-H)
      F  Within-arm change (SF)          (was SF8-C, imported)
      G  Within-arm change (AML+CAR-T)   (was SF8-D, imported)
      H  Cell counts per visit (SF)      (was SF8-G, imported)
    """
    print("Supplementary Figure 6: Participant Heterogeneity & Temporal Dynamics")
    apply_style()
    ...
    panels = [
        ...
        # D: Cross-dataset heterogeneity scatter (was SF7-G)
        (
            "panel_D",
            lambda fig, ax: _panel_cross_dataset_sd(ax, results),
            (8, 7),
        ),
        # E: Inter-feature correlation matrix (SF) (was SF7-H)
        (
            "panel_E",
    ...
    # Cleanup heterogeneity data
    for res in results.values():
        if "adata" in res:
            del res["adata"]
    results.clear()

    # ── Panels F-H: temporal panels from SF8 ──────────────────────────
    from .supp_fig8_temporal import (
        FEATURES as TEMP_FEATURES,
    )
    from .supp_fig8_temporal import (
        _load_all as _load_all_temporal,
    )
    from .supp_fig8_temporal import (
        _panel_cell_counts,
        _panel_within_arm_change,
        _panel_within_arm_change_multi,
    )

    temp_results = _load_all_temporal()
    if temp_results:
        sf_t = temp_results.get("Sade-Feldman", {})

        # Panel F: Within-arm change (SF) (was SF8-C)
        try:
            ...
            _panel_within_arm_change(
                ...
                sf_t.get("features", []),
                sf_t["cfg"],
                "Within-Arm Change (Sade-Feldman)",
            )
            fig.tight_layout()
            save_panel(fig, "panel_F", FIGURE_NAME, SUPP_OUTPUT)
        except Exception as exc:
            print(f"    panel_F: ERROR {exc}")
            plt.close("all")

        # Panel G: Within-arm change (AML + CAR-T) (was SF8-D)
        try:
            combined_feats = [
                f
                for f in TEMP_FEATURES
                if any(f in temp_results.get(ds, {}).get("features", []) for ds in ["AML", "CAR-T"])
            ]
            ...
            _panel_within_arm_change_multi(
                ...
                ["AML", "CAR-T"],
                combined_feats,
                "Within-Arm Change (AML + CAR-T)",
            )
            fig.tight_layout()
            save_panel(fig, "panel_G", FIGURE_NAME, SUPP_OUTPUT)
        except Exception as exc:
            print(f"    panel_G: ERROR {exc}")
            plt.close("all")

        # Panel H: Cell counts (SF) (was SF8-G)
        try:
            fig, ax = plt.subplots(figsize=(8, 6))
            _panel_cell_counts(
                ax, sf_t["adata"], sf_t["cfg"], "Cell Counts per Participant x Visit (Sade-Feldman)"
            )
            fig.tight_layout()
            save_panel(fig, "panel_H", FIGURE_NAME, SUPP_OUTPUT)
        except Exception as exc:
            print(f"    panel_H: ERROR {exc}")
            plt.close("all")

        # Cleanup temporal data
        for res in temp_results.values():
            if "adata" in res:
                del res["adata"]
        temp_results.clear()

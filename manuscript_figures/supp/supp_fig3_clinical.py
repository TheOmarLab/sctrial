...
def generate():
    ...
    Reorganised layout (6 panels):
      A  Design summary table            (was SF3-A)
      B  Cells per participant × arm     (was SF3-C)
      C  Baseline PCA by arm             (was SF3-D)
      D  Baseline gene detection by arm  (was SF3-E)
      E  Baseline cell-type by arm       (was SF3-F)
      F  Sensitivity / MDE               (was SF4-D, imported)
    """
    print("Supplementary Figure 3: Baseline Comparability & Trial Design")
    loaded = _load_all()
    ...
    # Panel B: Cells per participant per arm (was SF3-C)
    ...
    # Panel D: Baseline gene detection by arm (was SF3-E)
    fig, ax = plt.subplots(figsize=(9, 5))
    _panel_baseline_ngenes(ax, loaded)
    fig.tight_layout()
    save_panel(fig, "panel_D", FIGURE_NAME, SUPP_OUTPUT)

    # Panel E: Baseline cell-type composition by arm (was SF3-F)
    ...
    if did_results:
        ...
        _panel_sensitivity(ax, did_results)
        fig.tight_layout()

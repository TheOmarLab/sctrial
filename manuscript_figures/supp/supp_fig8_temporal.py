...
# Panels from this script are now generated by supp_fig7 (new SF6).
# This module is retained for its panel functions which are imported.
FIGURE_NAME = "SuppFig6_heterogeneity_temporal"  # merged into SF6
...
def _panel_pre_post_scatter(ax, pb: pd.DataFrame, features: list[str], cfg: dict, title: str):
    ...
    for feat in main_feats:
        texts.append(
            ax.text(pre_means[feat], post_means[feat], feat, fontsize=7, ha="left", va="bottom")
        )
    if texts:
        adjust_text(
            texts,
    ...
    if outlier_feats:
        ...
        for feat in outlier_feats:
            out_strs.append(f"{feat}: Pre={pre_means[feat]:.2f}, Post={post_means[feat]:.2f}")
        ax.text(
            0.97,
            0.03,
            "Outlier (off-plot):\n" + "\n".join(out_strs),
    ...
    ax.text(
        ...
        va="top",
        bbox=dict(
            facecolor="white", alpha=0.9, edgecolor="gray", linewidth=0.5, boxstyle="round,pad=0.3"
        ),
    )
...
def _panel_within_arm_change(
    ax, pb: pd.DataFrame, features: list[str], cfg: dict, title: str, *, note: str | None = None
):
...
def _panel_within_arm_change_multi(
    ax, results: dict, dataset_names: list[str], features: list[str], title: str
):
...
def _panel_effect_vs_baseline(ax, pb: pd.DataFrame, features: list[str], cfg: dict, title: str):
    ...
    if outlier_idx:
        ...
        for i in outlier_idx:
            out_strs.append(f"{avail[i]}: baseline={baselines[i]:.2f}, effect={effects[i]:.2f}")
        ax.text(
            0.97,
            0.03,
            "Outlier (off-plot):\n" + "\n".join(out_strs),
...
def _panel_cell_counts(ax, adata, cfg: dict, title: str):
    ...
    counts = (
        adata.obs.groupby([pid_col, visit_col, arm_col], observed=True)
        .size()
        .reset_index(name="n_cells")
    )
    counts.rename(
        columns={pid_col: "participant_id", visit_col: "visit", arm_col: "arm"}, inplace=True
    )
...
def _panel_cross_dataset_fc(ax, results: dict):
    ...
    common = [
        f for f in common if np.isfinite(fc_data[names[0]][f]) and np.isfinite(fc_data[names[1]][f])
    ]
    if len(common) == 0:
        ax.text(0.5, 0.5, "No shared features", transform=ax.transAxes, ha="center", va="center")
        ax.axis("off")
        return
    ...
    if len(names) >= 3:
        ...
        common3 = [
            f
            for f in common3
            if np.isfinite(fc_data[names[0]][f]) and np.isfinite(fc_data[names[2]][f])
        ]
        if len(common3) > 0:
    ...
    ax.plot([-lim, lim], [-lim, lim], "--", color="gray", linewidth=1.0, alpha=0.6)
...
def generate():
    """Panels from this module are now generated by supp_fig7 (new SF6).

    This generate() is a no-op; panel functions are imported directly
    by supp_fig7_heterogeneity.generate().
    """
    print("Supplementary Figure 8: panels merged into SF6 — skipping standalone.")
    print("  Run supp_fig7_heterogeneity.generate() to produce all SF6 panels.")
    return
